...
- **Никакого тихого success-drop входящего внешнего события.** Любая ветка обработчика webhook/callback внешнего провайдера, которая ОТБРАСЫВАЕТ входящее событие без обработки, но возвращает success-код (2xx) — `not_an_object`, `missing_<id>`, неизвестный `event_type`, дедуп-скип и т.п. — ОБЯЗАНА залогировать причину структурированным логом (уровень warning+, с идентификатором/типом события). Провайдер трактует 2xx как успешную доставку и не ретраит, поэтому принятое-но-необработанное событие обязано быть видимым в логах/метриках. Тихий success-drop (2xx без лога причины) на денежном/биллинговом пути недопустим — это невидимый прод-инцидент.
...
- [ ] Обработчик webhook/callback внешнего провайдера: каждая ветка, отбрасывающая событие без обработки, но возвращающая 2xx (`missing_<id>`, `not_an_object`, неизвестный `event_type`, дедуп-скип), логирует причину (warning+, с id/типом события) — нет тихого success-drop, особенно на денежном пути
