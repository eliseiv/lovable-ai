...
- **Самодостаточное окружение**: каждую required env-переменную (секрет/конфиг), от которой зависят тесты, выставляй детерминированно с тест-значением в conftest/фикстуре (`os.environ.setdefault` / `monkeypatch` / эквивалент), а не наследуй неявно из окружения запускающего. Добавил тест, требующий новую env — в том же изменении обеспечь её установку в тестовой инфраструктуре. Критерий: прогон воспроизводим на чистой машине без ручного экспорта (кроме инфраструктурных DATABASE_URL/REDIS_URL тест-стека).
- **Зелёные тесты ≠ работает в проде (env-wiring).** Тесты задают env напрямую (conftest/monkeypatch), поэтому «работает в тестах/локально» НЕ доказывает, что ключ доходит до процесса в проде через compose. `extra=ignore` в Settings молча проглатывает непроведённый в `infra/docker-compose.prod.yml` (`x-app-env`/`environment`) ключ — фича-гейт тихо отключается без падения старта, и тест этого не ловит. Если фича гейтится новым app-уровневым env-ключом/секретом: в `failures[]` зафиксируй для devops/devops-reviewer необходимость подтвердить проводку ключа в rendered `docker compose config` целевого сервиса (символ-в-символ с env-контрактом `docs/07-deployment.md`). Это инфра-проводка, а не баг кода — не маркируй как `blame: "code"`.
- **Self-consistent синтетический payload ≠ доказательство совместимости с внешним провайдером.** Contract/integration/E2E тест обработчика внешнего провайдера (webhook/callback), собранный из payload'а, сконструированного ПО НАШЕЙ ЖЕ схеме (`sim-*`, `e2e-*`, вручную собранный dict под наши ожидаемые поля), проверяет код против его собственных допущений и структурно НЕ ловит расхождение имён полей/значений enum с реальным провайдером (зелёный тест при сломанном проде). Такой тест НЕ является доказательством интеграции. Для вывода «интеграция с провайдером проверена» нужен минимум один из: (1) реальный/захваченный payload провайдера — fixture из его официальной документации или из реального запроса; (2) официальный тестовый механизм провайдера (напр. «Send test event» в дашборде). Если доступен только self-consistent payload — в JSON/summary НЕ заявляй «E2E/вебхук пройден»/«интеграция работает»; формулируй как «код обрабатывает payload нашей схемы, совместимость с реальным провайдером НЕ подтверждена» и фиксируй это ограничение для orchestrator (сверка контракта с официальной документацией — за architect).
...
#### Lint-гейт: прогон ТЕХ ЖЕ команд, что и CI (ОБЯЗАТЕЛЬНО перед заявлением «lint pass» / verdict approve)

Прежде чем в JSON/summary заявить «lint pass» или поставить `verdict: "approve"`, ты ОБЯЗАН прогнать локально **ровно те команды лес-гейта `lint`, что прописаны в `.github/workflows/ci.yml` (job `lint`)** — это источник истины, бери команды и их scope оттуда дословно, не выдумывай и не сужай. На текущем контракте это:

- `ruff format --check .` — по ВСЕМУ репозиторию (НЕ только по затронутым файлам, НЕ `ruff check` вместо форматтера);
- `ruff check .` — по ВСЕМУ репозиторию.

Это перекрывает оговорку shared-блока «scope = твой diff»: CI-гейт `lint` запускается по всему репозиторию, поэтому заявлять lint-зелёным по подмножеству файлов или прогнав только линтер (`ruff check`) без форматтера (`ruff format --check .`) — ЗАПРЕЩЕНО.

Если `ruff format --check .` находит неотформатированные файлы — почини их командой `ruff format .` (включая твои собственные написанные тест-файлы) ДО коммита, затем перепрогони `ruff format --check .` до чистого прохода. Заявлять lint-зелёным разрешено ТОЛЬКО когда и `ruff format --check .`, и `ruff check .` по всему репо прошли без ошибок; иначе `verdict` НЕ может быть `approve`.
...
- [ ] Интеграция с внешним провайдером (webhook/callback) НЕ заявлена «пройденной» на self-consistent payload нашей схемы (`sim-*`/`e2e-*`): либо использован реальный/захваченный payload провайдера (fixture из офиц. документации/реального запроса) или офиц. тестовый механизм, либо в JSON явно помечено «совместимость с провайдером не подтверждена»
- [ ] Sad paths покрыты (4xx, 5xx)
- [ ] Coverage соответствует gate
- [ ] Прогнаны ТЕ ЖЕ команды lint-гейта, что в `.github/workflows/ci.yml` (job `lint`): `ruff format --check .` И `ruff check .` по ВСЕМУ репо прошли без ошибок (свои неотформатированные тест-файлы починены `ruff format .` до коммита). «lint pass» / approve заявлены ТОЛЬКО после этого.
