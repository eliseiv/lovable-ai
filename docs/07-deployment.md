...
| `ADMIN_API_KEY` | `admin_api_key` | SecretStr\|None | api | **ADR-021.** Секрет админ-плоскости (`X-Admin-Key` для `require_admin` на `/v1/admin/*`: login-as, начисление бонус-кредитов). Сравнивается constant-time (`hmac.compare_digest`). **Пусто/None → админ-плоскость отключена** (`require_admin` всегда `401`). Работает в **dev И prod** (безопасность — секрет, не среда; `environment` не участвует). Секрет уровня root-доступа (login-as = вход за любого юзера), encrypted-at-rest, в Sentry scrubbed ([05-security → Админ-плоскость](05-security.md#админ-плоскость-adr-021), [modules/admin/03-architecture.md](modules/admin/03-architecture.md)). ⚠️ **backend/devops: добавить в Settings + secret-manager (api).** | `` (пусто = отключено) |
...
| `BUILD_EGRESS_SUBNET` | — (compose-only, **НЕ** поле `Settings`) | str (CIDR) | compose | **Мульти-инстанс.** Статический IPAM-subnet сети `build_egress` (`ipam.config.subnet` в compose). Приложение его НЕ читает (в `app/core/config.py` НЕ добавляется). Параметризован для изоляции IPAM-пулов между инстансами — клон задаёт свободный /16, не пересекающийся с занятыми bridge-пулами хоста (иначе Docker `Pool overlaps`). Prod-дефолт (corelysite) — `172.31.0.0/16`; клон — своё (см. [Мульти-инстанс / клон сервиса](#мульти-инстанс--клон-сервиса-второй-инстанс-за-тем-же-edge-traefik-adr-018-мульти-инстанс)). **Согласование со squid:** src-ACL build-клиентов egress-proxy — НЕ per-instance `${BUILD_EGRESS_SUBNET}`, а единый `172.16.0.0/12`, покрывающий любой инстансный /16 (защита держится сетевой изоляцией, [ADR-010 §C-2](adr/ADR-010-build-sandbox-rootless-egress.md#c-2-source-acl-build-клиентов-мульти-инстанс-нормативно-прод-фикс-2026-06-04) / [egress-proxy ACL build-клиентов](#egress-proxy-acl-build-клиентов-мульти-инстанс-нормативно--прод-фикс-2026-06-04)). | `172.31.0.0/16` |
...
## Провижининг build-workspace и sites-каталога (host-bind, path-consistency) — прод-фикс (2026-06-04)

> **Прод-инцидент (2026-06-04):** первый реальный prod-build упал на `[Errno 13] Permission denied: '/var/builds'` — каталог `BUILDS_ROOT` **отсутствовал** в worker-контейнере и не провижился (worker-сервис монтировал только rootless `docker.sock` + APNs-секрет; `Dockerfile.worker` каталог не создавал). Контракт `BUILDS_ROOT`/`SITES_HOST_ROOT` ([env-контракт](#канонический-список-ключей)) и эфемерность workspace ([ADR-010 §B](adr/ADR-010-build-sandbox-rootless-egress.md)) были зафиксированы, но **топология их провижининга** (где физически живёт каталог, как он становится доступен worker-контейнеру И rootless-демону по **одному и тому же абсолютному пути**, кто его создаёт и с какими правами) — не была. Этот раздел — нормативная топология провижининга; он не вводит новых решений, а делает исполняемыми уже принятые `BUILDS_ROOT`/`SITES_HOST_ROOT`/[ADR-010](adr/ADR-010-build-sandbox-rootless-egress.md)/[ADR-002](adr/ADR-002-nginx-mount-vs-baked.md) для shared-single-host prod ([ADR-018](adr/ADR-018-prod-deployment-shared-traefik-cicd.md)).

### Инвариант path-consistency (nested rootless `docker -v`)

Ключевой инвариант, диктующий всю топологию ниже: **bind-source путь во вложенном `docker run -v {src}:{dst}`, который worker делает к rootless-демону, резолвит rootless-демон относительно ФС хоста, где этот демон работает, — НЕ относительно ФС worker-контейнера.** Поэтому когда build-воркер запускает `docker run -v {builds_root}/{job_id}:/workspace` ([ADR-010 §B](adr/ADR-010-build-sandbox-rootless-egress.md)) или nginx-сайт `docker run -v {sites_host_root}/{pid}:/usr/share/nginx/html:ro` ([ADR-002](adr/ADR-002-nginx-mount-vs-baked.md)), путь `{builds_root}/{job_id}` / `{sites_host_root}/{pid}` обязан указывать на **один и тот же физический каталог**, видимый по **тому же абсолютному пути** во всех трёх контекстах:

1. **worker-контейнер** — пишет в него (распаковка `source.tgz`, складывает `dist/`); путь = значение `BUILDS_ROOT` / `SITES_HOST_ROOT` внутри контейнера;
2. **хост rootless-демона** — по этому же абсолютному пути физически лежит каталог;
3. **вложенный build/nginx-контейнер** — монтирует его как `/workspace` / `/usr/share/nginx/html`.

**Следствие (нормативно):** worker обязан bind-mount'ить host-каталог `BUILDS_ROOT`/`SITES_HOST_ROOT` в worker-контейнер по **идентичному абсолютному пути** (`-v /var/builds:/var/builds`, `-v /srv/sites:/srv/sites`), а **не** под произвольной внутриконтейнерной точкой. Несовпадение путей → вложенный `docker run -v` смонтирует несуществующий/чужой путь хоста (пустой `/workspace` или `dist/` 404), даже если worker-контейнер пишет «успешно». Это та же логика, по которой rootless `docker.sock` монтируется через `${ROOTLESS_DOCKER_SOCK}` (хостовый путь), а не системный сокет.

### Топология `BUILDS_ROOT` (`/var/builds`) — build-workspace

| Аспект | Норматив (shared-single-host prod, [ADR-018](adr/ADR-018-prod-deployment-shared-traefik-cicd.md)) |
|---|---|
| **Где физически живёт** | Host-каталог на build-хосте, в пространстве, доступном **rootless-демону** (он бежит под непривилегированным служебным пользователем). Канонический путь = значение `BUILDS_ROOT` (дефолт `/var/builds`). Если служебный пользователь rootless-демона не имеет прав на `/var/builds` — devops либо выдаёт ему права на этот путь (см. ownership ниже), либо переопределяет `BUILDS_ROOT` на путь в его домашнем/`$XDG`-пространстве (например `/home/<rootless-user>/builds`) и синхронно правит host-bind worker'а — значение должно совпасть символ-в-символ с тем, что видит демон. |
| **Bind в worker** | `-v ${BUILDS_ROOT}:${BUILDS_ROOT}` (идентичный абсолютный путь, см. инвариант выше) в `worker`-сервисе `docker-compose.prod.yml`. **НЕ** именованный volume — нужен реальный host-путь, видимый rootless-демону. |
| **Ownership / права** | Каталог принадлежит так, чтобы **worker uid 10001 (`app`) писал**, а **rootless-демон читал/монтировал**. Практически: владелец/группа покрывают обоих, либо `0775`+общая группа, либо ACL. Создаётся **до** старта worker (см. шаг провижининга). |
| **Эфемерность / cleanup** | Содержимое `{BUILDS_ROOT}/{job_id}` — эфемерный workspace, очищается воркером после сборки (успех/фейл), как в S1 ([ADR-010 §B](adr/ADR-010-build-sandbox-rootless-egress.md)). Сам host-каталог `BUILDS_ROOT` — persистентный (создаётся один раз провижинингом). |
| **Дисковая квота** | Каталог под дисковой квотой ([ADR-010 §B](adr/ADR-010-build-sandbox-rootless-egress.md)) — защита от runaway-сборок забить диск; механизм квоты (fs-quota / отдельный том) — на стороне build-хоста, devops. |

### Топология `SITES_HOST_ROOT` (`/srv/sites`) — статика сайтов

| Аспект | Норматив |
|---|---|
| **Где физически живёт** | Host-каталог на deploy-хосте, по абсолютному пути = значение `SITES_HOST_ROOT` (дефолт `/srv/sites`), доступный rootless-демону (тот же демон деплоит nginx-сайт-контейнеры). |
| **Bind в worker** | `-v ${SITES_HOST_ROOT}:${SITES_HOST_ROOT}` (идентичный абсолютный путь) в `worker`-сервисе. Worker пишет `dist/` в `{SITES_HOST_ROOT}/{pid}/`; nginx-контейнер монтирует `{SITES_HOST_ROOT}/{pid}` как `/usr/share/nginx/html:ro` ([ADR-002](adr/ADR-002-nginx-mount-vs-baked.md)). |
| **Ownership / права** | **worker uid 10001 пишет** `dist/`; **nginx читает** (`:ro`-mount). nginx:alpine отдаёт статику от своего пользователя — права каталога должны допускать чтение монтируемым контейнером (`o+rx` на путь или общая группа). Создаётся **до** старта worker. |
| **Жизненный цикл** | Каталог `{SITES_HOST_ROOT}/{pid}/` persистентен пока сайт LIVE; teardown/GC проекта ([ADR-011](adr/ADR-011-project-delete-gc.md)) удаляет его вместе с контейнером/route. Сам host-каталог `SITES_HOST_ROOT` — persистентный. |

### Шаг провижининга host-каталогов (кто создаёт, когда, с какими правами)

**Нормативное требование:** host-каталоги `BUILDS_ROOT` и `SITES_HOST_ROOT` **создаются ДО старта worker-сервиса** с ownership, позволяющим worker uid 10001 писать, а rootless-демону — читать/монтировать. Это часть отложенного **live-приёмочного пункта «rootless+egress энфорс на реальном build-хосте»** ([Прод-топология](#прод-топология) load-test build-фермы). Допустимы два механизма (devops выбирает; первый — рекомендуемый для shared single-host):

1. **Host-провизион в deploy-процедуре (рекомендуется).** На build/deploy-хосте (`/opt/corelysite`, [ADR-018](adr/ADR-018-prod-deployment-shared-traefik-cicd.md)) перед `docker compose … up` создаются каталоги с нужным владельцем — один раз при первичной настройке хоста (владелец сервера/devops) либо идемпотентно в deploy-шаге CI **до** `compose up`. Каталоги версионируются не файлами, а **процедурой** (документируется в deploy-runbook / CI deploy-job рядом с `git pull`). Это согласуется с тем, что rootless-демон и его `$XDG_RUNTIME_DIR`/сокет (`ROOTLESS_DOCKER_SOCK`) уже провижатся владельцем хоста вне compose.
2. **Compose bind + init-обёртка.** Bind host-пути в worker + одноразовый init (entrypoint-обёртка worker'а или отдельный one-shot-сервис), который при старте делает `mkdir -p` и `chown` каталога **до** запуска celery. Ограничение: worker бежит non-root (uid 10001) — `chown` чужого каталога ему недоступен; поэтому init, меняющий ownership, требует либо root-фазы в entrypoint (до `USER app`), либо чтобы каталог уже существовал с верными правами (тогда init лишь `mkdir -p` подкаталогов). Для shared-single-host это усложнение, поэтому **дефолт — механизм 1** (host-провизион в процедуре).

> **Связь с `Dockerfile.worker`:** сам `BUILDS_ROOT`/`SITES_HOST_ROOT` в образе worker создавать **не нужно** (и недостаточно — внутрисозданный каталог не виден rootless-демону по хост-пути; см. инвариант path-consistency). Создание точек bind-mount (`/var/builds`, `/srv/sites`) Docker делает автоматически при mount'е. Требование к образу — лишь чтобы процесс под uid 10001 имел право писать в **примонтированный** каталог (обеспечивается ownership host-каталога, не Dockerfile).

### Не ломать существующие инварианты

Этот провижининг — дополнение к уже зафиксированным границам, ничего из них не отменяет: rootless-демон ([ADR-010](adr/ADR-010-build-sandbox-rootless-egress.md), `ROOTLESS_DOCKER_SOCK`); egress-изоляция build-контейнера (отдельная сеть `BUILD_EGRESS_NETWORK`/`build_egress` + egress-proxy, [ADR-010 §C](adr/ADR-010-build-sandbox-rootless-egress.md)) — build-workspace bind НЕ открывает build-контейнеру лишний доступ (монтируется только `{builds_root}/{job_id}` как `/workspace`, не весь `BUILDS_ROOT`); сети worker (`default` + `web` + `build_egress`); path-routing сайтов (`/s/{site_id}`, [ADR-017](adr/ADR-017-path-based-site-routing.md)). Bind-каталоги — обычные host-bind, в build_egress-изоляцию не входят.

## Деплой сайта (build-воркер)

1. `source.tgz` из S3 → распаковка в эфемерный `/var/builds/{job_id}` (host-каталог `BUILDS_ROOT`, провижится по [§ Провижининг build-workspace](#провижининг-build-workspace-и-sites-каталога-host-bind-path-consistency--прод-фикс-2026-06-04)).
2. `npm ci && vite build` в throwaway build-контейнере. **S1:** запуск через привилегированный `docker.sock`, базовая изоляция (cap-drop ALL, non-root, ресурс-лимиты) — [TD-001](100-known-tech-debt.md#td-001). **S4:** rootless Docker + egress-allowlist (npm-registry only) + seccomp/no-new-privileges — [ADR-010](adr/ADR-010-build-sandbox-rootless-egress.md), нормативная конфигурация запуска [05-security.md → «Конфигурация запуска build-контейнера»](05-security.md#конфигурация-запуска-build-контейнера-sprint-4-нормативная), «[Модель изоляции сборки по спринтам](#модель-изоляции-сборки-по-спринтам)». Под `--read-only`+non-root build-контейнер ОБЯЗАН получить писучий HOME + tool-cache внутри `/workspace` (env `HOME`/`npm_config_cache`/`XDG_CACHE_HOME`/`XDG_CONFIG_HOME` → подкаталоги `/workspace`, [ADR-010 §B-2](adr/ADR-010-build-sandbox-rootless-egress.md)) — это **фиксированные пути-константы внутри контейнера** (НЕ новые operator-env в [env-контракте](#канонический-список-ключей)), без них `npm ci` падает на `ENOENT … mkdir '/.npm'` (read-only rootfs, HOME=`/`). **В режиме `SITE_ROUTING_MODE=path`** воркер добавляет `vite build --base=/s/{site_id}/` (нормативно — [modules/deploy/03-architecture.md §2A](modules/deploy/03-architecture.md#2a-path-based-routing-s-site_id-prod--site_routing_modepath-adr-017), [ADR-017](adr/ADR-017-path-based-site-routing.md)); без base-path ассеты за StripPrefix отдают 404.
3. `dist/` → `/srv/sites/{pid}/` (host-каталог `SITES_HOST_ROOT`, провижится по [§ Провижининг build-workspace](#провижининг-build-workspace-и-sites-каталога-host-bind-path-consistency--прод-фикс-2026-06-04); путь монтирования у worker = хост-путь = путь, видимый rootless-демону). `docker run -d --name site_{subdomain} --restart unless-stopped` `nginx:alpine` с примонтированным `dist/` (ro) и Traefik-лейблами (generic nginx + mount, см. [ADR-002](adr/ADR-002-nginx-mount-vs-baked.md)). Лейблы зависят от режима: `subdomain` → Host(`{subdomain}.apps.domain`); `path` → `Host({APPS_DOMAIN}) && PathPrefix(/s/{site_id})`+StripPrefix+priority ([modules/deploy/03-architecture.md §2A](modules/deploy/03-architecture.md#2a-path-based-routing-s-site_id-prod--site_routing_modepath-adr-017), prod-фикс [ADR-017 §Fix](adr/ADR-017-path-based-site-routing.md#fix-2026-06-03--host-обязателен-в-path-правиле-прод-инцидент)). Сеть — `{TRAEFIK_NETWORK}` (prod = `web`). Флаг `--restart unless-stopped` — источник restart-политики, который снимает teardown (`docker rm -f`); соответствует `run_nginx_container` (см. [modules/deploy/03-architecture.md → §3](modules/deploy/03-architecture.md#3-deploy-generic-nginx--mount)).
...
- **`worker`** (`-Q llm` и/или `-Q build`) + **`beat`** — `default` (+ worker: `web` + `build_egress`); build-воркер сохраняет доступ к Docker для деплоя сайт-контейнеров. Сайт-контейнеры в prod деплоятся в сеть **`web`** (чтобы общий Traefik их видел) — `TRAEFIK_NETWORK=web`. **Worker-volumes (нормативно):** помимо rootless `docker.sock` (`${ROOTLESS_DOCKER_SOCK}:/var/run/docker.sock`) и APNs-секрета, worker **обязан** host-bind'ить build-workspace и sites-каталог по **идентичному абсолютному пути** — `-v ${BUILDS_ROOT}:${BUILDS_ROOT}` и `-v ${SITES_HOST_ROOT}:${SITES_HOST_ROOT}` — иначе вложенный rootless `docker run -v` не видит каталог (path-consistency, [§ Провижининг build-workspace](#провижининг-build-workspace-и-sites-каталога-host-bind-path-consistency--прод-фикс-2026-06-04)). Host-каталоги создаются **до** старта worker с ownership «uid 10001 пишет / rootless-демон читает».
...
4. **Явные имена сетей `build_egress` / `egress_uplink`.** Текущие `name: corelysite_build_egress` и `name: corelysite_egress_uplink`. Параметризовать в **`name: ${BUILD_EGRESS_NETWORK:-corelysite_build_egress}`** (ключ `BUILD_EGRESS_NETWORK` уже существует в x-app-env с тем же дефолтом — теперь его значение используется и в блоке `networks:`, символ-в-символ) и в новый env-ключ **`name: ${EGRESS_UPLINK_NETWORK:-corelysite_egress_uplink}`**. Причина: build-egress-сети инстансов **обязаны** быть РАЗНЫМИ (инстансы не делят egress-границу build-песочницы); общее имя сети → инстансы делили бы одну Docker-сеть. `BUILD_EGRESS_NETWORK` (x-app-env, передаётся воркеру) и `name:` сети `build_egress` обязаны иметь **одно значение** — иначе воркер создаёт build-контейнеры в одной сети, а compose объявляет другую.
   ...
   - **Статический IPAM-subnet `build_egress` тоже параметризуется.** Сеть `build_egress` объявлена со **статическим** пулом (`ipam.config.subnet`, текущий `172.31.0.0/16`). Два инстанса на одном хосте с одним и тем же статическим пулом → Docker отвергает создание сети клона: `invalid pool request: Pool overlaps with other one on this address space` (прод-инцидент 2026-06-04 при первом деплое `nexoraweb`). Поэтому subnet выносится в новый env-ключ **`name: subnet: ${BUILD_EGRESS_SUBNET:-172.31.0.0/16}`** (дефолт = текущий corelysite-пул → backward-compat). Клон задаёт **свободный** /16 в своём `.env` (напр. `172.30.0.0/16`), не пересекающийся с занятыми на хосте (`docker network inspect` существующих bridge-сетей). **Новый env-ключ `BUILD_EGRESS_SUBNET`** — compose-only (НЕ поле `Settings`), str (CIDR), дефолт `172.31.0.0/16`.
...
| `BUILD_EGRESS_SUBNET` | свободный /16, напр. `172.30.0.0/16` | Статический IPAM-пул сети `build_egress` клона. ОБЯЗАН не пересекаться с пулами существующих bridge-сетей хоста (corelysite держит `172.31.0.0/16`), иначе Docker отвергает создание сети (`Pool overlaps`). |
...
- **`egress-proxy`/`squid.conf` НЕ параметризуется по инстансу** (см. ниже): один статический конфиг для обоих инстансов; src-ACL build-клиентов покрывает весь Docker-bridge диапазон `172.16.0.0/12` → допускает build-контейнеры любого `BUILD_EGRESS_SUBNET` без правки конфига.

### egress-proxy ACL build-клиентов (мульти-инстанс, нормативно — прод-фикс 2026-06-04)

> **Прод-инцидент (деплой клона `nexoraweb`).** `infra/egress-proxy/squid.conf` содержит `acl build_clients src 172.31.0.0/16` — захардкоженную build-egress-подсеть `corelysite`. `BUILD_EGRESS_SUBNET` параметризован (строка env-контракта выше: клон задаёт `172.30.0.0/16`), а **парный** ему src-ACL squid — нет. `squid.conf` монтируется одним статическим файлом из репозитория для **обоих** инстансов (`./egress-proxy/squid.conf:/etc/squid/squid.conf:ro`, без envsubst). Build-контейнер клона (`src 172.30.0.4`) ∉ `172.31.0.0/16` → squid `TCP_DENIED/403 CONNECT registry.npmjs.org:443` → `npm error E403` → сборка падает (`no_progress`). На `corelysite` (172.31) работает.

**Решение (нормативно, [ADR-010 §C-2](adr/ADR-010-build-sandbox-rootless-egress.md#c-2-source-acl-build-клиентов-мульти-инстанс-нормативно-прод-фикс-2026-06-04)): src-ACL build-клиентов в `infra/egress-proxy/squid.conf` расширяется до `acl build_clients src 172.16.0.0/12`** — единый CIDR на весь RFC1918-диапазон Docker-bridge подсетей (покрывает `172.31` corelysite, `172.30` nexoraweb и любой будущий build-egress /16 инстанса). Конфиг остаётся **одним статическим файлом** для всех инстансов; envsubst/шаблон/новый env-ключ **не вводятся**.

- **Безопасность не страдает.** Src-ACL `build_clients` — defence-in-depth поверх **первичной** сетевой изоляции: `egress-proxy` слушает `3128` только на `build_egress`(`internal:true`)+`egress_uplink` и наружу порт НЕ публикует (нет `ports:`), а build-контейнер сидит только в `build_egress`. Физически достучаться до proxy могут лишь build-контейнеры **данного** инстанса — независимо от ширины src-ACL. Семантика контроля — «только build-контейнеры», а не «конкретная подсеть»; эффективное множество источников не меняется. Точность ACL заменяется сетевой изоляцией + поясняющим комментарием в `squid.conf`.
- **Обратная совместимость.** `172.31.0.0/16` (corelysite) ⊂ `172.16.0.0/12` → редеплой `corelysite` ничего не меняет; клон проходит ACL автоматически (его /16 ⊂ /12).
- **Не конфликтует с anti-SSRF.** `acl to_private dst 172.16.0.0/12` + `http_access deny to_private` остаются без изменений — это `dst` (запрет адресата, анти-SSRF/DNS-rebind), а `build_clients` — `src` (источник). Расширение `src` не ослабляет `dst`-фильтр.
- **`NPM_REGISTRY_ALLOWLIST`/`acl npm_registry dstdomain` не затрагиваются** — меняется только src-ACL.
