...
| `STUCK_THRESHOLD_S` | `stuck_threshold_s` | int | worker | Порог «зависания» джобы в `BUILDING/DEPLOYING/FIXING` для reconciler'а (crash-resume). | `900` |
...
- Лейбл сайта **зависит от `SITE_ROUTING_MODE`**:
  ...
  - `path` (prod): `PathPrefix("/s/{site_id}")` + StripPrefix-middleware + `entrypoints=websecure`; нормативно — [modules/deploy/03-architecture.md §2A](modules/deploy/03-architecture.md#2a-path-based-routing-s-site_id-prod--site_routing_modepath-adr-017) ([ADR-017](adr/ADR-017-path-based-site-routing.md)).
...
- **`deploy`:** SSH (`SSH_HOST`/`SSH_USER`/`SSH_PRIVATE_KEY`) → `cd /opt/corelysite` → `git pull` → `docker compose -f docker-compose.prod.yml up -d --build`. Чужие `/opt/music-backend`, `/opt/edge` не трогаются.
