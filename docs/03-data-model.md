...
| `requested_locale` | text NULL | **[ADR-036] — явный locale-override языка генерации.** Нормализованный BCP-47 язык, запрошенный клиентом в Form-поле `locale` при `POST /v1/projects` (`'ru'`/`'en'` через новую `normalize_locale`); **`NULL` = locale не передан/неподдерживаемый → авто-детект** из `project.prompt` ([ADR-028](adr/ADR-028-deterministic-source-prompt-language-detection.md)). В `_interview`: задан ⇒ `language = language_from_bcp47(requested_locale)` (приоритет, без `detect_language`); `NULL` ⇒ `detect_language(project.prompt)` (байт-в-байт как прежде). Фиксируется в `generation_jobs.content_language` как сегодня. Прод-фикс: iOS-boilerplate (латиница) перевешивал русский запрос → `en`. Нормативно — [ADR-036](adr/ADR-036-explicit-client-locale-override.md), [pipeline §Язык/локализация](modules/pipeline/03-architecture.md#языклокализация-контента-сайта--детерминированный-детект-adr-028-ревизует-adr-025). **Миграция:** транзакционный `add_column('projects', requested_locale text NULL)` (НЕ autocommit_block — нет non-transactional DDL; [ADR-031](adr/ADR-031-alembic-sync-engine-non-transactional-ddl.md)), `down_revision = "20260616_0001"`, без backfill (поле nullable, существующие проекты = `NULL` = авто-детект). |
...
| `retry_of_job_id` | text FK→generation_jobs NULL | **[ADR-053] — цепочка повторов упавшей генерации.** `NULL` = обычный запуск; непустое значение = джоба создана ручкой `POST /v1/jobs/{jid}/retry` вместо упавшей (`retry_of_job_id` указывает на неё). Повтор живёт **в том же проекте**, поэтому промпт, `requested_locale`, `template_id`/`style_id`/`model_id` и `attachments` берутся с проекта без копирования; `questions`/`answers` принадлежат джобе и копируются в новую. Списание генерации повтор **не** делает — вместо этого при создании пишется `job_events.usage_counted` с `payload.source='retry'`, и `count_generation_start` в фазе интервью становится no-op ([ADR-053 §B](adr/ADR-053-failed-generation-retry.md)); число бесплатных повторов ограничено `GENERATION_RETRY_MAX_ATTEMPTS` — счёт ведётся как число джоб проекта с непустым `retry_of_job_id`. Партиальный индекс не нужен: обычный индекс `(retry_of_job_id)` обслуживает и поиск существующего повтора (идемпотентность), и счёт цепочки. **Миграция:** транзакционный `add_column` nullable-колонки без `server_default` + self-FK + индекс ([ADR-031](adr/ADR-031-alembic-sync-engine-non-transactional-ddl.md), sync psycopg env.py), `down_revision = "20260908_0001"`, backfill не нужен (существующие джобы = `NULL`). |
...
| `event_type` | text | `state_changed`, `agent_started`, `question_posted`, `build_failed`, `fix_attempted`, `deployed`, `failed`, `edit_requested`, ... Для fix-loop: `build_failed` (с `payload.failure_class`/`failure_signature`), `fix_attempted` (с `payload.retry_count`). **Sprint 5:** `deploy_infra_unavailable` ([ADR-055](adr/ADR-055-deploy-attempt-row-and-infra-failures.md)) — деплой не выполнен по причине уровня хоста (пул адресов сети исчерпан, демон/диск/сеть); джоба идёт в терминальный `infra_error` без fix-loop, `payload` несёт `detail` и `failure_log_ref`. `upload_refs_checked` ([ADR-054](adr/ADR-054-edit-photos-manifest-and-ref-repair.md)) — отчёт серверной починки ссылок на загруженные фото (`prefixes_fixed`, `renamed`, `unused_new`, `unknown_left`), пишется только когда было что чинить или фото осталось неиспользованным. `edit_requested` (`kind='edit'`) несёт в `payload` текст правки пользователя (`payload.instruction`) — **единственный** источник хранения instruction (отдельной колонки `generation_jobs.instruction` нет, см. примечание под `generation_jobs`). Несёт историю витков: `build_failed.payload.failure_log_ref` (build/deploy-фейл) и `fix_rejected.payload.failure_log_ref` (отклонённый патч Agent 4 → `agent.{retry_count}.log`) каждого события указывают на per-attempt лог именно той попытки/стадии ([ADR-022](adr/ADR-022-per-attempt-build-logs.md)) — `generation_jobs.failure_log_ref` хранит лишь ссылку **последней** записанной попытки/стадии, а полная история per-attempt ссылок восстанавливается из append-only `build_failed`/`fix_rejected`-событий. |
| `from_state` / `to_state` | text NULL | Для переходов. |
| `payload` | jsonb | Доп. данные события. |
| `created_at` | timestamptz | |

## job_sections (план сайта, [ADR-046](adr/ADR-046-site-plan-and-section-progress.md))

План генерации и его выполнение: что агент собирается построить и что уже построено.
Строки создаёт фаза `SPECCING` из поля `sections` вывода Agent 2; `status` переводится в
`done`, когда идентификатор секции распознан в потоке кодогенерации Agent 3.

| Поле | Тип | Заметки |
|---|---|---|
| `id` | bigserial PK | |
| `job_id` | text FK→generation_jobs | Индекс по `(job_id, position)`. |
| `section_id` | text | Латиница kebab-case; уникален в пределах джобы (`uq_job_sections_job_section`). Он же токен детекта прогресса. |
| `title` | text | Название на языке контента — показывается пользователю. |
| `position` | int | Порядок секции на странице. |
| `status` | text | `pending` \| `done`. Промежуточного `in_progress` нет: поток даёт момент готовности секции, а не её начала. |
| `completed_at` | timestamptz NULL | Момент перевода в `done`. |
| `created_at` | timestamptz | |

> **Отсутствие строк — не ошибка.** У правок, откатов и задач, созданных до появления
> планов, плана нет: `GET /jobs/{jid}/plan` отдаёт пустой список, клиент показывает чат
> без чек-листа. План — вспомогательный UX-канал, а не артефакт сборки: его отсутствие
> никогда не влияет на исход генерации.
...
|---|---|---|
| `id` | bigserial PK | |
| `adapty_event_id` | text UNIQUE NOT NULL | **Идемпотентность** обработки вебхуков. **Ключ дедупа** = `event_properties.profile_event_id` (UUID) → synthetic fallback (`adapty-syn:{event_type}:{transaction_id}` → `adapty-syn:body:{sha256}`), [ADR-040 §A](adr/ADR-040-adapty-webhook-dedup-key-event-type-reconciliation.md) (ревизует ADR-027 §C `event_id||id`). Выводится ВСЕГДА → тихого дропа `missing_event_id` нет. Единственная точка дедупа; повтор → `200` no-op. Исторические значения (`sim-*`/`e2e-*`/`manual-*`) — backward-compat, без миграции ([ADR-040 §E](adr/ADR-040-adapty-webhook-dedup-key-event-type-reconciliation.md)). |
| `event_type` | text | `KNOWN_EVENT_TYPES` = 18 фактических типов Adapty (сверено с first-party доке 2026-07-10, [ADR-040 §C](adr/ADR-040-adapty-webhook-dedup-key-event-type-reconciliation.md)): handled (`subscription_started`/`subscription_renewed`/`subscription_renewal_cancelled`/`subscription_expired`/`subscription_refunded`/`billing_issue_detected`/`access_level_updated`), consumable (`non_subscription_purchase`), consciously-ignored (`subscription_renewal_reactivated`/`subscription_paused`/`subscription_deferred`/`trial_*`(5)/`entered_grace_period`/`non_subscription_purchase_refunded`). Маппинг handled → `subscriptions` — нормативно [modules/billing/03-architecture.md §2.3](modules/billing/03-architecture.md#23-маппинг-event_type--subscriptions-нормативная-таблица); классификация — [§2.5](modules/billing/03-architecture.md#25-known_event_types-и-наблюдаемость-отброшенных-событий-adr-040-cd). |
...
**Движок миграций — sync psycopg.** Alembic `env.py` гонит миграции **синхронным** движком psycopg по env-ключу `DATABASE_URL_SYNC` (`postgresql+psycopg://`), а НЕ async asyncpg (рантайм приложения остаётся на asyncpg/`DATABASE_URL`). Только на sync-движке штатный alembic-механизм non-transactional DDL работает. Стек/ключ — [02-tech-stack.md](02-tech-stack.md) / [07-deployment.md → env](07-deployment.md#канонический-список-ключей).

**Паттерн.** Non-transactional DDL оборачивается в `op.get_context().autocommit_block()` (на sync-движке он реально переводит соединение в AUTOCOMMIT и приостанавливает транзакцию миграции) + по возможности идемпотентная форма:

```python
def upgrade() -> None:
    with op.get_context().autocommit_block():
        op.execute("ALTER TYPE job_state ADD VALUE IF NOT EXISTS 'NEW_VALUE'")
```

- **`IF NOT EXISTS` (идемпотентность).** Обязательно для `ADD VALUE`: безопасно на БД, где значение уже создано (повторный прогон / ручное досоздание на проде → no-op).
- **`downgrade` для `ADD VALUE` — no-op:** `ADD VALUE` в PG необратим (удаление значения требует пересоздания типа с перезаписью зависимых колонок).

**Почему `autocommit_block()` поверх async-движка НЕ работал (корень инцидента 2026-06-12).** Прежний `env.py` гнал alembic через `async_engine_from_config` (asyncpg) + `connection.run_sync(...)`. `autocommit_block()` рассчитан на синхронный DBAPI-движок и поверх asyncpg+`run_sync` **НЕ** переводит реальное asyncpg-соединение в AUTOCOMMIT → `ADD VALUE 'EDITING'` исполнялся внутри транзакции и не фиксировался видимо, тогда как `alembic_version` коммитился → DDL «потерян» при пройденном гейте (лог «Will assume transactional DDL»). Перевод движка на sync psycopg ([ADR-031](adr/ADR-031-alembic-sync-engine-non-transactional-ddl.md) §A) убирает класс этих багов.

**Тест-требование.** Тест enum/non-transactional миграции обязан проверять РЕАЛЬНОЕ применение DDL (наличие значения в `pg_enum` после `alembic upgrade head` на прод-движке/конфиге), а не только `alembic_version` — [06-testing-strategy.md](06-testing-strategy.md), [ADR-031 §D](adr/ADR-031-alembic-sync-engine-non-transactional-ddl.md).
