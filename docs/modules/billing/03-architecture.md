...
```mermaid
flowchart TB
    A[Adapty webhook S2S] --> B{верификация секрета/подписи Adapty}
    B -->|fail| E[401]
    B -->|ok| C{adapty_event_id в billing_events?}
    C -->|да| OK1[200 идемпотентно, no-op]
    C -->|нет| D[insert billing_events processed_at=NULL]
    D --> M{маппинг customer_user_id → user}
    M -->|неизвестен| NU[user_id=NULL, processed_at=NULL → 200, алерт]
    M -->|найден| U[apply: subscriptions.status/access_level/expires/grace_until]
    U --> P[processed_at = now, в одной транзакции]
    P --> OK2[200]
```

### 2.1 Верификация
Секрет/подпись Adapty (`ADAPTY_WEBHOOK_SECRET`), **не Bearer**. Секрет — secret manager/env ([05-security §Adapty webhook](../../05-security.md#adapty-webhook-не-bearer)). Невалидно → `401`.
...
|---|---|---|---|---|
| `subscription_started` | `active` | из `profile.access_level` | `NULL` | из `subscription.*` |
| `subscription_renewed` | `active` | из профиля | `NULL` | из payload |
| `access_level_updated` | `active` (если профиль активен), иначе по правилам ниже | новое значение | без изменения | без изменения |
| `subscription_expired` | `grace` | сохраняется текущий до teardown | `expires_at + GRACE_PERIOD_DAYS` | `will_renew=false` |
...
| `subscription_renewed`/`started` когда текущий `status ∈ {grace, billing_issue}` | `active` | из профиля | **`NULL`** (отмена pending-teardown, §6) | из payload |
