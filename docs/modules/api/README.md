...
- 2026-06-17: **явный locale-override ([ADR-036](../../adr/ADR-036-explicit-client-locale-override.md), закрывает [Q-LOCALE-1](../../99-open-questions.md#q-locale-1)) реализован** — опц. Form-поле `locale` (BCP-47) в multipart `POST /v1/projects`: неподдерживаемое/пустое значение → fallback на серверный авто-детект (**не** `422`), переопределяет script-детект языка контента. Контракт — [02-api-contracts → POST /projects](02-api-contracts.md#post-projects); `/edits` не меняется (наследует язык сайта через маркер). Без новых env-ключей. Нормативный раздел контракта уже консистентен с реализацией (reviewer approve) — здесь статусная отметка (architect).
