...
| GET | `/models` | каталог моделей генерации ([ADR-051](../../adr/ADR-051-user-selectable-generation-model.md)) | Bearer | `200` |
...
- **Content-Type: `multipart/form-data`** ([ADR-034](../../adr/ADR-034-user-image-attachments-vision-site-assets.md)) — было `application/json`, **breaking change для iOS**. Form-поля: `prompt` (обяз.), `title` (опц.), `locale` (опц., [ADR-036](../../adr/ADR-036-explicit-client-locale-override.md)); файлы: `images` — `list[UploadFile]` (опц., 0..`MAX_IMAGES_PER_JOB`).
...
- **Язык контента сайта — опциональный явный `locale` ([ADR-036](../../adr/ADR-036-explicit-client-locale-override.md)) с fallback на авто-детект.** Опц. Form-поле `locale` (BCP-47-подобное: `ru`, `ru-RU`, `en`, `en-US`, …) переопределяет язык генерации. Нормализация → `ru`/`en` (первый сабтег до `-`/`_`, регистронезависимо); **неподдерживаемое (`fr`/`de`/…) или пустое → fallback на авто-детект из `prompt`, НЕ `422`** (значение трактуется как «locale не передан»). Без `locale` → авто-детект из **исходного** `prompt` ([ADR-028](../../adr/ADR-028-deterministic-source-prompt-language-detection.md) ревизует [ADR-025](../../adr/ADR-025-content-language-autodetect-spec-marker.md), [pipeline §Язык/локализация](../pipeline/03-architecture.md#языклокализация-контента-сайта--детерминированный-детект-adr-028-ревизует-adr-025)) — байт-в-байт как прежде. Хранится в `projects.requested_locale` ([03-data-model → projects](../../03-data-model.md#projects)). **Scope — только генерация:** контракт `POST /projects/{pid}/edits` `locale` НЕ принимает (правка наследует язык сайта через маркер `**Content language:**`, [ADR-036 §2](../../adr/ADR-036-explicit-client-locale-override.md)). Поле опционально → обратносовместимо.
...
- **`model_id`** (Form, опц., [ADR-051](../../adr/ADR-051-user-selectable-generation-model.md)) — модель из `GET /models`. Применяется ко всем шагам генерации и **запоминается на проекте**: правки (`POST /projects/{pid}/edits`) и откаты идут на той же модели.
  - Без значения каждый агент работает со своим `AGENTn_MODEL` инстанса (они различаются: спека пишется более сильной моделью).
  - Неизвестный `model_id` (в том числе id из каталога другого провайдера) → **`422`**.
  - Стоимость для пользователя не зависит от выбора: списание — 1 генерация (`cost_tokens`), лимиты те же.

## GET /models ([ADR-051](../../adr/ADR-051-user-selectable-generation-model.md))
Каталог моделей генерации — «чем строим сайт». Выбранный пресет применяется ко **всем** шагам генерации (агентам `agent1`…`agent4`).
- `200` → `{ "items": [ { "id", "title", "description" } ] }`; порядок = порядок карточек. Сейчас два пресета: `fast` и `quality`.
- **Состав зависит от провайдера инстанса** (`LLM_PROVIDER`): под одним и тем же `id` на anthropic- и openai-инстансе стоят разные модели, поэтому идентификаторы берутся только из этого ответа.
- Пустой `items` — выбор на инстансе недоступен, генерация идёт на значениях по умолчанию.
