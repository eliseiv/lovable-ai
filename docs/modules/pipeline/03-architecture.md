...
|---|---|---|---|
| **Agent 1 (Interviewer)** | промт + **серверная language-директива** (язык из `content_language`, §Язык/локализация) | список уточняющих вопросов → `questions`, на языке директивы | llm |
| **Agent 2 (Spec writer)** | промт + ответы + **серверная language-директива** (§Язык/локализация) | финальная спека (`spec_tz`/`spec_ref`); `spec_markdown` **обязан** начинаться маркером `**Content language:**` со значением из директивы (§Язык/локализация) | llm |
