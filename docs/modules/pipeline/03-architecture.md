...
> **Терминальные состояния — `LIVE` и `FAILED`** (`_TERMINAL_JOB_STATES` в [events.py](../../app/pipeline/events.py)). Диаграмма (текст/таблица переходов ниже / `transition()`) согласованы: **нет ни одного перехода ИЗ `FAILED`**; единственный «выход» из `LIVE` — `LIVE → FIXING` по `POST /edits`, и это **новая edit-джоба той же ревизии**, а не перезапись терминала исходной джобы (edit-цикл, [ADR-014](../../adr/ADR-014-edit-limit-revision-rollback.md)). Машинно терминал = «`state` не перезаписывается ничем» — см. §Инвариант терминальности ниже.
