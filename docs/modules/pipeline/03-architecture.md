...
| `infra_error` | исчерпан Celery `max_retries` на транзиентном инфра-сбое (ADR-006) |
...
- **Исчерпание Celery `max_retries`** на инфра-сбое → `FAILED(infra_error)` (вина окружения, не кода сайта).
...
### E2. Reconciler застрявших `FIXING`/`DEPLOYING`/`BUILDING` (crash-resume)
- **Назначение:** подхватить джобы, зависшие в активных нетерминальных состояниях после краша воркера/beat между переходами (NFR crash-resumable). Дополняет `acks_late` (acks_late покрывает потерю **таски**; reconciler покрывает джобы, у которых **нет** живой таски в очереди — например, краш между транзакцией смены state и постановкой следующей таски).
- **Stuck-критерий:** `state ∈ {BUILDING, DEPLOYING, FIXING}` И `updated_at < now() - STUCK_THRESHOLD_S` (default **900 s** = 15 мин — заведомо больше нормального витка сборки+health). Порог не должен быть меньше суммарного wall-clock одного витка, иначе ложно «оживим» нормально работающую джобу.
- **Действие:** для stuck-джобы диспетчер **переставляет таску по текущему `state`** (тот же механизм, что подхват после рестарта, ADR-001) — не меняет `state`, а ре-диспетчеризует. Идемпотентно благодаря cleanup-before-run в deploy. Гарды проверяются на обычном пути (если за время простоя истёк `wall_clock_deadline` → reconciler-перепостановка приведёт к штатному `FAILED(wall_clock_exceeded)`).
- **Частота:** beat каждые **120 s** (`RECONCILE_INTERVAL_S`).
- **Защита от дабл-постановки:** ре-диспетчеризация под `SELECT … FOR UPDATE SKIP LOCKED` + проверка, что таска для этого `(job_id, state)` ещё не активна (по короткоживущему Redis-lock `dispatch:{job_id}`), чтобы reconciler и `acks_late`-повтор не поставили две таски.

> Reconciler уместен для Sprint 2 (вместе с fix-loop появляются длинные многовитковые джобы — выше шанс зависнуть в `FIXING`/`DEPLOYING`). Включён в DoD Sprint 2.
