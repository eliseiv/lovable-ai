...
flowchart TB
    ...
    B --> C[npm install && npx vite build в песочнице -- path-режим: + --base=/s/site_id/]
...
- **Routing (Traefik-labels на сайт-контейнере, path-режим):**
  - `traefik.enable=true`
  - `traefik.http.routers.{site_id}.rule=Host("{APPS_DOMAIN}") && PathPrefix("/s/{site_id}")` — **`Host(...)` обязателен (prod-фикс [ADR-017 §Fix](../../adr/ADR-017-path-based-site-routing.md#fix-2026-06-03--host-обязателен-в-path-правиле-прод-инцидент)).** В prod сайт-контейнеры живут во **внешней общей сети `web`** рядом с чужими сервисами под общим edge-Traefik ([ADR-018](../../adr/ADR-018-prod-deployment-shared-traefik-cicd.md)). Правило **без** `Host(...)` (`PathPrefix` в одиночку) матчит **любой** `Host` на edge-Traefik, включая чужие домены/сервисы → перехват чужих запросов и конфликт роутеров. `Host("{APPS_DOMAIN}")` ограничивает правило доменом приложения (`APPS_DOMAIN`, prod = `corelysite.shop`); ключ конфига — `APPS_DOMAIN` ([07-deployment.md env](../../07-deployment.md#канонический-список-ключей)).
  - `traefik.http.routers.{site_id}.priority={SITE_ROUTER_PRIORITY}` — **явный приоритет обязателен.** На общем edge-Traefik рядом существует router API (`Host("corelysite.shop")`, [07-deployment.md → Traefik](../../07-deployment.md#traefik-dev--свой-traefik-prod--общий-edge-traefik)). Запрос `corelysite.shop/s/{site_id}/...` обязан матчиться сайт-роутером (более специфичное правило `Host && PathPrefix`), а не API-роутером (`Host` только). Traefik по умолчанию ранжирует по длине правила, но **полагаться на это нельзя** при сосуществовании с чужими роутерами → задаём приоритет явно через `SITE_ROUTER_PRIORITY` (выше, чем у catch-all `Host`-роутера API). Ключ — `SITE_ROUTER_PRIORITY` ([07-deployment.md env](../../07-deployment.md#канонический-список-ключей)).
  ...
  > **Нормативный источник prod-инцидента «`PathPrefix` без `Host`»:** `app/deploy/routing.py` формировал path-режимное правило как `PathPrefix("/s/{site_id}")` без `Host(...)` и без `priority` → на общей сети `web` правило матчило чужие запросы и конфликтовало с соседними сервисами. Канон (этот §2A + [ADR-017 §Fix](../../adr/ADR-017-path-based-site-routing.md#fix-2026-06-03--host-обязателен-в-path-правиле-прод-инцидент)): правило **всегда** `Host("{APPS_DOMAIN}") && PathPrefix("/s/{site_id}")` + явный `priority={SITE_ROUTER_PRIORITY}`. В режиме `subdomain` (dev) правило остаётся `Host("{subdomain}.apps.domain")` — там `Host` уже присутствует, инцидент его не касается.
- **Site build base-path (НОРМАТИВНО, критично).** Сгенерированный Vite-сайт **ОБЯЗАН** собираться с `base=/s/{site_id}/`, иначе за StripPrefix ассеты (`/assets/*`, картинки) резолвятся в корень `corelysite.shop/assets/...` и отдают 404. `site_id` генерируется при создании строки `site_deployments` **до** фазы build (см. поток выше), поэтому известен на момент сборки.
  ...
  - **Нормативный способ вызова vite — `npx vite build` (НЕ голый `vite build`, НЕ `npm run build`).** Воркер нормализует `build.command` из дерева Agent 3/Agent 4 к канонической форме перед запуском в build-контейнере (нормализация — по образцу уже существующего `npm ci`→`npm install`, см. §3 / [07-deployment.md → Деплой сайта](../../07-deployment.md#деплой-сайта-build-воркер)):
    - голый `vite build` → `npx vite build` (голый `vite` ищется в `$PATH` и отсутствует там — vite лежит в `node_modules/.bin`, не в `PATH` → `sh: vite: not found` → ложный `build_error` + лишний fix-loop);
    - `npm run build` → `npx vite build` (npm-script находит vite, **но** не прокидывает добавленный воркером хвостовой `--base` во вложенный vite без разделителя `--`, см. ниже);
    - `npx vite build` → без изменений.
    - `npx vite build` находит локальный vite из `node_modules/.bin` (устраняет `vite: not found`). **Скачивания нет:** vite уже установлен предшествующим `npm install`; build-сеть — internal + egress-allowlist `NPM_REGISTRY_ALLOWLIST` (`registry.npmjs.org`), [ADR-010 §C](../../adr/ADR-010-build-sandbox-rootless-egress.md). Итоговая build-команда в path-режиме = `npm install && npx vite build --base=/s/{site_id}/`.
    - Унификация на `npx vite build` (а не сохранение кастомного `npm run build`-script) **обоснована** в [ADR-017 §Fix (2026-06-08)](../../adr/ADR-017-path-based-site-routing.md#fix-2026-06-08--vite-not-found--потеря---base-через-npm-run-build-прод-инцидент): надёжность инжекта base + устранение `vite: not found` важнее сохранения кастомных pre-build шагов script'а (`tsc &&` и т.п. — vite сам транспилит TS, потеря приемлема).

  - **Механизм инжекта `--base` (НОРМАТИВНО, критично — точка прод-инцидента 2026-06-08).** base инжектится **именно в токен `npx vite build`**, а не дописывается в хвост всей строки команды. То есть `--base=/s/{site_id}/` добавляется как аргумент `vite`, давая `… && npx vite build --base=/s/{site_id}/`, а **не** `… && npx vite build … --base` после неотносящегося к vite сегмента. Дописывание в хвост строки до нормализации (`npm run build --base=...`) — корень прод-инцидента: npm трактует `--base` как флаг **самого npm**, не прокидывает его в vite (для этого нужен разделитель `npm run build -- --base`), и vite собирает с дефолтным `base=/` → ассеты `/assets/...` → 404 за StripPrefix → пустой экран. Инжект на токен `npx vite build` устраняет зависимость от порядка `&&`-сегментов и от наличия `--`-разделителя. CLI-флаг (а не правка `vite.config` в LLM-дереве) — base не зависит от недоверенного кода и инжектится воркером (как `.npmrc`/proxy, [05-security threat-model](../../05-security.md#threat-model-центр--build-sandbox)). В режиме `subdomain` base не задаётся (сайт в корне хоста, дефолт `/`).
  - Это единственный нормативный источник требования base-path и способа вызова vite; pipeline (Agent 3 output) собственного base не задаёт — base — забота фазы build (deploy), не генерации дерева. Эталонный `build.command` в выводе Agent 3/Agent 4 — `npm install && npx vite build` ([pipeline → Контракт output Agent 3](../pipeline/03-architecture.md#контракт-output-agent-3-полная-валидируемая-схема)).

  > **Нормативный источник прод-инцидента «пустой экран path-сайта» (2026-06-08):** HTML сгенерированного сайта ссылался на ассеты по абсолютному пути `/assets/index-*.js` (вместо `/s/{site_id}/assets/...`) → `GET {domain}/assets/...` → 404 (catch-all API-роутер `Host(domain)`) → JS не исполнялся → пустой `<div id="app">`. Две причины в цепочке: (1) `vite build` без `npx` → `vite: not found`; (2) «починка» Agent 4 на `npm run build` + хвостовой `--base` → npm не прокидывал base → `base=/`. Канон (этот §2A + [ADR-017 §Fix (2026-06-08)](../../adr/ADR-017-path-based-site-routing.md#fix-2026-06-08--vite-not-found--потеря---base-через-npm-run-build-прод-инцидент)): нормализация к `npx vite build` + инжект `--base` на токен `npx vite build`.
