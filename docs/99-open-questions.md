...
## Q-DEPLOY-4
**CI/CD для второго (клон) инстанса.** Текущий GitHub Actions workflow деплоит **только** в `/opt/corelysite` ([07-deployment.md → CI/CD](07-deployment.md#cicd-контракт-github-actions-adr-018)). Нужно ли автоматизировать деплой второго инстанса (`nexoraweb` и далее) через CI — параметризовать workflow matrix по `{каталог, project-name, env-файл}` — или достаточно ручного деплоя.
**Решение (минимальное, [07-deployment.md → Мульти-инстанс](07-deployment.md#cicd-клона-решение)):** на старте клон деплоится **вручную** на сервере по нормативной [процедуре деплоя клона](07-deployment.md#процедура-деплоя-клона-нормативная); отдельный CI **не вводится**. Параметризация workflow под несколько таргетов — возможная будущая работа devops, не требуется для запуска `nexoraweb`.
**Блокирует:** **НЕ блокирует** запуск клона (ручной деплой самодостаточен). Ответ нужен только если потребуется автоматизировать деплой второго+ инстанса.
**Статус:** open (низкий приоритет; ручной деплой — нормативный baseline).
