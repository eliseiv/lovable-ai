...
| **openai (SDK)** | **>=1.60** (нижняя граница — Responses API + reasoning-модели; верхняя не пинится, uv резолвит актуальную мажорную **2.x** — на 2026-06-15 `2.41.1`) | **Альтернативный LLM-провайдер ([ADR-032](adr/ADR-032-llm-provider-abstraction-openai.md), `LLM_PROVIDER=openai`).** 4 агента на GPT-5-class через **Responses API** (`responses.create`/`stream`): `reasoning={"effort":…}` (аналог adaptive thinking/effort) + `max_output_tokens` (аналог `max_tokens` cap). Structured-output — **тот же текстовый `extract_json`-путь** ([ADR-020](adr/ADR-020-agent-structured-output-tool-use-tolerant-parse-retry.md)), native `text.format`/`json_schema` НЕ используется ([ADR-032 §3](adr/ADR-032-llm-provider-abstraction-openai.md)). Prompt caching — **автоматический** (без `cache_control`-блоков), cache-hit в `usage.input_tokens_details.cached_tokens`. SDK-исключения (`RateLimitError`/`APIConnectionError`/`APITimeoutError`/`InternalServerError`/`APIStatusError`/`AuthenticationError`/`PermissionDeniedError`/`BadRequestError`) — те же имена, что Anthropic, из top-level `openai`; классификатор `retry_policy` распознаёт исключения **обоих** SDK ([ADR-032 §5](adr/ADR-032-llm-provider-abstraction-openai.md)). **Прямая зависимость** — объявить в `pyproject.toml` явно (оба SDK всегда в образе; провайдер выбирается рантаймом одного Docker-образа, **не** условным импортом). |
| OpenAI-модели | GPT-5.5-class per-agent в конфиге (env `AGENTn_MODEL`) | **Tiering OpenAI** ([ADR-032 §2](adr/ADR-032-llm-provider-abstraction-openai.md), provider-специфичные значения тех же `AGENTn_MODEL`; верифицировано 2026-06-15): дефолты Agent 1/3/4 = `gpt-5.4-mini` (дешёвый), Agent 2 = `gpt-5.5` (качество). reasoning.effort: Agent 1/2 — `OPENAI_AGENT_EFFORT` (дефолт `high`, из {`medium`,`high`,`xhigh`}); Agent 3/4 — `none` (наименьший уровень реального набора `none`/`low`/`medium`/`high`/`xhigh`; весь `max_output_tokens` под вывод полного file-tree, перенос мотива [ADR-023](adr/ADR-023-agent3-token-budget-thinking-room.md)). Model-IDs operator-tunable через env `AGENTn_MODEL` без релиза. ([Q-LLM-1](99-open-questions.md#q-llm-1) resolved.) |
| OpenAI pricing-таблица | per-1M USD `input`/`cached_input`/`output`; `cache_write`=`0` | **Аналог `_MODEL_PRICING` для OpenAI** (cost-ledger). **Нормативные числа (единственный источник) — [observability §2.2A](modules/observability/03-architecture.md#22a-openai-pricing-провайдер-openai-adr-032)** (верифицированы по каталогу OpenAI 2026-06-15, [Q-LLM-1](99-open-questions.md#q-llm-1) resolved). `cache_write`=`0` (caching автоматический, без write-ставки). Неизвестная модель → консервативный fallback (как Anthropic-fallback на Opus-тариф). |
...
| StoreKit 2 signed transactions (JWS) | — | **[ADR-039](adr/ADR-039-direct-storekit-jws-purchase-path.md).** **Прямой** канал покупок **параллельно** Adapty: iOS шлёт подписанную StoreKit 2 JWS-транзакцию на `POST /v1/tokens/purchase` / `POST /v1/subscription/sync`, backend верифицирует её **офлайн собственными средствами** (`cryptography` x5c-chain → Apple root + `PyJWT[crypto]` ES256, обе — выше в «Безопасность (библиотеки)»). Покрывает **Xcode StoreKit Testing** (где Adapty-вебхук структурно недоступен) + Sandbox/Production. **Отдельного SDK/API-клиента не вводит** (верификация офлайн). Модуль — `app/billing/storekit.py`, [modules/billing/03-architecture.md §13](modules/billing/03-architecture.md#13-прямой-storekit-путь-adr-039). |
...
| PyJWT[crypto] | **>=2.9** (актуальная мажорная 2.x; extra `crypto` тянет `cryptography`) | RS256-верификация Apple identity token по JWKS (`https://appleid.apple.com/auth/keys`, кэш по `kid`) в модуле `auth` (`app/auth/apple_verify`). Проверка подписи + `iss`/`aud`/`exp`/`nbf`/`nonce`. [ADR-007](adr/ADR-007-sign-in-with-apple.md), [modules/auth/03-architecture.md §1](modules/auth/03-architecture.md). **[ADR-039](adr/ADR-039-direct-storekit-jws-purchase-path.md):** переиспользуется для **ES256**-верификации подписи StoreKit 2 JWS-транзакции (`jwt.decode(key=<leaf pubkey>, algorithms=["ES256"], options={"verify_aud": False})`) в `app/billing/storekit.py` — extra `crypto` покрывает ES256, новой библиотеки не вводит. **Прямая зависимость** — обязана быть объявлена в `pyproject.toml` явно (extra `crypto`), **не** полагаться на транзитивную доступность `cryptography` через redis/другие пакеты. |
| cryptography | **>=43** (совместима с `PyJWT[crypto]`; референс-паттерн зафиксирован на `43.x`, точный pin — devops) | **[ADR-039](adr/ADR-039-direct-storekit-jws-purchase-path.md).** Верификация x5c cert-chain StoreKit 2 JWS в `app/billing/storekit.py`: `x509.load_der_x509_certificate` (загрузка leaf/intermediate/root), проверка «каждый cert подписан следующим» через `ec.ECDSA`/`padding.PKCS1v15` public-key verify, терминация цепочки в доверенном Apple root (сравнение DER-отпечатков + verify-signed-by). **Используется НАПРЯМУЮ** (`from cryptography import x509`, `cryptography.hazmat.primitives.asymmetric.ec/rsa/padding`, `serialization.Encoding`) → по правилу прямых зависимостей объявляется **явной прямой** зависимостью в `pyproject.toml`, **не** транзитив через `PyJWT[crypto]`. Ранее присутствовала лишь транзитивно (extra `crypto` PyJWT) — ADR-039 переводит в явную. ⚠️ **backend/devops: добавить `cryptography>=43` в `pyproject.toml` явной строкой.** |
| httpx | **0.27.x** | Async HTTP-клиент (Adapty, health-check сайтов, fetch JWKS Apple). **Sprint 5:** **extra `http2`** обязателен для APNs (`httpx[http2]` → тянет `h2`) — APNs Server API работает **только** по HTTP/2 ([ADR-013](adr/ADR-013-apns-push-from-job-events.md)). Прямая зависимость — объявить extra `http2` в `pyproject.toml` явно, **не** полагаться на транзитивный `h2`. **[ADR-035](adr/ADR-035-llm-stream-read-timeout.md):** `httpx.Timeout(read=…, connect=…)` задаёт **read/idle-таймаут LLM-клиентов** (`AsyncAnthropic(timeout=…)`/`AsyncOpenAI(timeout=…)`) — повисший («молчащий») SSE-стрим агента падает `APITimeoutError` за `LLM_READ_TIMEOUT_S` (транзиентный ретрай) задолго до stuck-порога; новой зависимости не вводит (оба SDK используют httpx-транспорт и принимают `httpx.Timeout`-объект). |
