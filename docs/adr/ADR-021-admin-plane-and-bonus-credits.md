...
### C. Подача админ-эндпоинтов в публичной OpenAPI — `include_in_schema=False`

- Все админ-эндпоинты (`POST /v1/admin/*`, `GET /v1/admin/*`) — **`include_in_schema=False`** (скрыты из `/openapi.json` и `/docs`), как `/metrics` и `/healthz` (B.5). Обоснование: админ-плоскость **не** предназначена iOS-клиенту; скрытие — простейший способ гарантировать чистоту публичной схемы (B.7 denylist) без риска утечки внутренних маркеров и без отдельного тега «Администрирование» в публичном справочнике.
- Денилист B.2 на эти эндпоинты формально не действует (они не в схеме), но **внутренние docstring/`summary`** админ-роутов всё равно пишутся **на русском, без `Sprint`/`ADR`/`TD`** — на случай, если когда-либо потребуется отдельная админ-схема (отозвать `include_in_schema=False`). Это страховка, не текущее требование.
...
- **(+)** Публичная Swagger чиста (админ скрыт), B.7 не нарушается.
...
- **404 при пустом ключе (вместо 401):** отвергнуто — требует условного монтирования роутера по конфигу (docs↔код-расхождение «эндпоинт то есть, то нет»); `include_in_schema=False` уже скрывает эндпоинты из схемы, а 401-always даёт единый код-путь.
