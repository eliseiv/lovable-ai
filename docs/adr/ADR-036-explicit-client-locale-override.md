...
**Единый нормативный источник правила приоритета** — [pipeline §Язык/локализация п.2](../modules/pipeline/03-architecture.md#языклокализация-контента-сайта--детерминированный-детект-adr-028-ревизует-adr-025). ADR-036 §6 фиксирует **решение** (script-детект промпта становится fallback'ом под явный клиентский locale) и **ссылается** на этот источник, не переформулируя правило. ADR-028 §4 и прочие представления так же ссылаются на pipeline §Язык/локализация п.2.
