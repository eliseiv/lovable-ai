...
- Системный промт каждого агента **обязан** содержать строгую инструкцию формата: **«Верни СТРОГО raw JSON нужной структуры. Без markdown-фенсов (` ``` `), без префиксов/пояснений/прозы до или после. Первый символ ответа — `{` или `[`.»** Эта инструкция — нормативная часть промта каждого из 4 агентов (общий шаблон `STRICT_JSON_SUFFIX` в `structured.py`).
- **(REVISION 2026-06-04) Строгий суффикс задаёт только ФОРМУ (raw JSON), но НЕ точные имена ключей — этого недостаточно.** Live-E2E вскрыл: в текстовом режиме модель **угадывает** имя top-level ключа-обёртки (Agent 2 трижды вернул валидный JSON под `spec`/`specification` вместо требуемого `spec_markdown` → валидатор → schema-фейл «empty» → `FAILED(invalid_agent_output)`). Поэтому промт каждого агента **дополнительно обязан декларировать СВОЮ точную output-схему** (точные top-level ключи + минимальный валидный пример), а валидатор обязан читать те же ключи. Нормативная каноническая схема каждого агента (single source of truth промт↔валидатор) и требование per-agent декларации — [pipeline §I.1a](../modules/pipeline/03-architecture.md#i1a-канонические-output-схемы-агентов--single-source-of-truth-нормативно) и [§I.6](../modules/pipeline/03-architecture.md). Дополнительно устранено противоречие в теле промта Agent 2 («respond as Markdown, do not wrap in JSON»), напрямую конфликтовавшее с JSON-суффиксом и валидатором `spec_markdown`.
