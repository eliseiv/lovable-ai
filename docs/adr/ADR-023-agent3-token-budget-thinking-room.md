...
| Agent 4 (Fixer/Editor) | sonnet-4-6 (≤64K) | **56 000** | `AGENT4_MAX_TOKENS` | 56000 | Возвращает то же дерево, что Builder (тот же риск усечения). thinking adaptive **сохранён** (диагностика фикса по логу ценна) — adaptive thinking делит cap с выводом, поэтому ≤ ceiling Sonnet с запасом обязателен. Cap **56000** (тот же запас от ceiling 64K, что и Agent 3); снижен с прежних 64000 (ревизия R1). |
...
- **Остальные агенты (1/2/4) сохраняют `thinking={"type":"adaptive"}`** — thinking ценен (Agent 2 — проектное мышление спеки; Agent 4 — анализ failure-лога; Agent 1 — формулировка вопросов). У них дефолтный `max_tokens` (16000–56000) с запасом вмещает adaptive thinking + вывод (каждый cap ≤ ceiling своей модели с запасом).
...
- **(A) ВЫБРАН — пер-агентный `max_tokens` (Builder 56000, ≤ ceiling Sonnet с запасом) + thinking-disabled у Builder, adaptive сохранён у 1/2/4; Builder на Sonnet 4.6 (R1).** Детерминированная комната под вывод + запас от усечения и от ceiling; минимально, в существующем механизме ADR-020; модель Builder env-переключаема. **Принят.**
