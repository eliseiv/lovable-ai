...
- **Неизвестный `vendor_product_id` при `non_subscription_purchase`** (product_id не в `TOKEN_PACK_PRODUCTS` / отсутствует): токены **не** начисляются (не угадываем). `billing_events` записывается с `processed_at=NULL` (событие не теряется — оператор может добавить маппинг и вручную переобработать), логируется warning/alert `billing_unknown_token_product`, ответ `200 {"status":"ignored","reason":"unknown_token_product"}`. Автопереобработки нет (getProfile-ресинк — только подписки); реобработка после правки env — ручная.
...
- **Q-BILLING-5** (сопутствующая находка, не в scope ADR-038) — официальный список типов Adapty содержит `subscription_renewal_cancelled`, тогда как ADR-027 §F / `KNOWN_EVENT_TYPES` использует `subscription_cancelled`. Требуется сверка с фактической конфигурацией webhook Adapty при интеграции (совместно с [Q-BILLING-4](../modules/billing/README.md)); при подтверждении расхождения — отдельная ревизия ADR-027, не ADR-038.
