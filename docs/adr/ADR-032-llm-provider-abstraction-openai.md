...
- **Обрыв Responses-стрима без `response.completed` (прод-фикс 2026-08-19, nexoraweb `j_vdn7eussn31o3ijpeuoss02g`).** `openai.lib.streaming.responses.get_final_response()` бросает **stdlib `RuntimeError`** («Didn't receive a `response.completed` event.»), когда HTTP 200 уже пришёл, но SSE закрылся без финального события. Это **не** `openai.APIError` → `is_transient`/`is_llm_failure` его не видели → Celery не ретраил → джоба висела в `SPECCING` до reconciler `stuck_timeout`. Решение (тот же паттерн, что anthropic-`TypeError` → `LLMCredentialError`): `OpenAIAgentClient._stream_final_response` **узко** перехватывает этот `RuntimeError` **только** на точке `get_final_response` (матч по `response.completed` в сообщении; прочий `RuntimeError` пробрасывается as-is) и поднимает доменный `IncompleteLLMStreamError`. Классификатор: `IncompleteLLMStreamError` ∈ `TRANSIENT_EXCEPTIONS` **и** `is_llm_failure` → Celery autoretry; исчерпание `max_retries` → `FAILED(agent_unavailable)`, **не** `infra_error`/`stuck_timeout`. SDK-исключения иерархии `openai.APIError` **по-прежнему не оборачиваются**.
