...
- **Preflight LLM-credential (основной путь, version-agnostic — см. §Fix(round 3)):** перед первым обращением к Anthropic SDK агент-таска проверяет наличие непустого LLM-credential; при отсутствии — немедленный graceful `FAILED(agent_unavailable)` **без** SDK-вызова и **без** ретраев.
...
- **Не-транзиентные** сбои (`AuthenticationError`/`401` — нет/невалиден ключ; `403`; `400`; а также client-side auth-resolution ошибка SDK — см. §Fix(round 3)) — **НЕ ретраятся**, немедленный graceful `FAILED(agent_unavailable)` без сжигания `max_retries`.
...
## Fix (2026-06-04, round 3) — fail-fast preflight LLM-credential до SDK-вызова (прод-инцидент)

> Прод-E2E (после round-2 Redis loop-fix) выявил **остаточный пробел graceful-fail (B)**: при **ПУСТОМ/отсутствующем `ANTHROPIC_API_KEY`** джоба `task_interview` всё равно зависала в `INTERVIEWING` ~15 мин и терминализировалась лишь reconciler-страховкой как `FAILED(wall_clock_exceeded)`/`stuck_timeout`, а не fail-fast как `FAILED(agent_unavailable)`. **Это уточнение действующего ADR-019** (тот же инвариант §G «недоступность LLM → быстрый graceful FAILED, не зависание»), а не новый ADR — корень иной, чем A/B/round-2, но симптом (concurrency-leak: слот занят до reconciler-TTL) и инвариант те же.

**Симптом.** При пустом `ANTHROPIC_API_KEY` Anthropic SDK бросает **встроенный** `TypeError("Could not resolve authentication method. Expected one of api_key, auth_token, or credentials to be set...")` на этапе сборки заголовков (`anthropic._client._validate_headers`) — **ДО** HTTP-запроса. Это **не** `anthropic.AuthenticationError` и **не** `APIStatusError`, а stdlib-`TypeError`. Поэтому текущий классификатор (`app/workers/retry_policy.py`: `is_transient` / `is_llm_failure` / `is_non_retryable_llm_failure`, все опираются на иерархию `anthropic.APIError`) его **НЕ** распознаёт как LLM-сбой → исключение идёт по ветке «unexpected» (Celery autoretry / re-raise) → graceful-fail (§G) **не** срабатывает → джоба висит в `INTERVIEWING` до reconciler по `STUCK_THRESHOLD_S=900 s`. Нарушение целевого инварианта §G «при недоступности LLM шаг агента делает graceful-переход в `FAILED` **fail-fast**, а не через 15 мин».

**Корневая причина.** Graceful-fail (B) полагался **исключительно** на классификацию исключений SDK. Но детерминированная недоступность credential проявляется **до** входа в HTTP-слой SDK как **встроенный `TypeError`** вне иерархии `APIError` — класс, который контракт §G не покрывал.

**Решение (нормативно — два слоя, основной + подстраховка):**

1. **Корневой фикс — per-job fail-fast preflight LLM-credential (основной, version-agnostic).** Каждая агент-таска (`task_interview`/`task_spec`/`task_build_request`/`task_fix`) **перед первым обращением к Anthropic SDK** проверяет наличие непустого `ANTHROPIC_API_KEY` (`Settings.anthropic_api_key`, [07-deployment.md → env-контракт](../07-deployment.md#контракт-переменных-окружения-environment-reference)). Пусто/`None`/whitespace-only → таска **немедленно** делает graceful-переход `FAILED(agent_unavailable)` **без** SDK-вызова, **без** ретраев (не сжигает `max_retries`), освобождая concurrency-слот. Преимущество: **не зависит от внутренних типов исключений SDK** (TypeError/AuthenticationError/версия SDK) — детерминированно ловит самый частый прод-кейс (сервис намеренно запущен без ключа). Нормативный контракт — [pipeline §G](../modules/pipeline/03-architecture.md#g-graceful-fail-шага-агента-при-недоступности-llm-adr-019).
2. **Подстраховка — расширить классификацию на client-side auth-resolution ошибку SDK.** `app/workers/retry_policy.py` дополнительно трактует встроенный `TypeError` SDK-этапа аутентификации («Could not resolve authentication method» / отсутствие `api_key`/`auth_token`/`credentials`) как **не-транзиентный LLM-сбой** → немедленный `FAILED(agent_unavailable)` без ретраев. Матч **узкий и устойчивый к версиям**: ловить класс `TypeError`, поднятый **синхронно при конструировании/вызове Anthropic-клиента до HTTP**, в идентифицируемой точке (фабрика клиента агента), а **не** по подстроке сообщения через весь стек (хрупко между версиями SDK). Это второй слой на случай невалидного-но-непустого credential, на котором SDK тоже может бросить client-side TypeError; основной путь (п.1) покрывает пустой ключ независимо.

**Уровень валидации — per-job, НЕ отказ старта сервиса (нормативно).** Сервис намеренно может стартовать **без** `ANTHROPIC_API_KEY` (текущая прод-реальность: app/worker обязаны подниматься без ключа, отдавать `/metrics`, обслуживать не-LLM-пути). Поэтому fail-fast — **per-job на старте шага агента**, а **не** валидация конфига при деплое (которая упала бы старт). Credential-валидации-на-старте процесса для `ANTHROPIC_API_KEY` **не вводится**.

**Следствие.** При пустом/невалидном/недоступном LLM-credential джоба уходит в `FAILED(agent_unavailable)` за **секунды** (на старте шага агента, до SDK-вызова), освобождая слот, **без** ожидания reconciler-TTL. Reconciler (§E2 ветвь 2, `FAILED(stuck_timeout)`) и wall-clock §C(c) остаются **вторым слоем** (backstop на смерть воркера), а **не** основным путём для этого класса. Без новой внешней зависимости — `anthropic` SDK уже в стеке ([02-tech-stack.md](../02-tech-stack.md)).
...
- **(round 3) Только расширение классификации SDK-`TypeError` (b) без preflight (a).** Отвергнут как основной путь: матч по типу/тексту встроенного `TypeError` хрупок между версиями SDK (формулировка сообщения и точка броска могут меняться), а самый частый прод-кейс — **пустой** ключ — детерминированно ловится preflight'ом без зависимости от SDK-внутренностей. Preflight (a) — основной (version-agnostic), классификация (b) — подстраховка для невалидного-но-непустого ключа.
- **(round 3) Отказ старта сервиса при отсутствии `ANTHROPIC_API_KEY` (fail-fast на деплое/конфиге).** Отвергнут: прод-реальность — сервис намеренно стартует без ключа (обязан подниматься, отдавать `/metrics`, обслуживать не-LLM-пути). Старт-валидация ключа сломала бы это. Канон — per-job fail-fast: сервис поднимается без ключа, но LLM-джобы падают быстро (§Fix round 3, п. «Уровень валидации»).
- **(round 3) Полагаться на reconciler/wall-clock как основной путь для пустого ключа.** Отвергнут: даёт `FAILED(wall_clock_exceeded)`/`stuck_timeout` через 15 мин и держит слот всё это время — деградация UX и concurrency-leak. Reconciler/wall-clock остаются backstop'ом, основной путь — preflight.
