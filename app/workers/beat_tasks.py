...
engine — иначе `RuntimeError: Future attached to a different loop` (asyncpg). Аналогично
async-Redis: `fail_job→transition→publish_event` дёргает Redis, поэтому тело оборачивается в
`worker_redis_scope()` — per-task Redis-клиент в этом же loop'е (ADR-019 §Fix; иначе
`RuntimeError: Event loop is closed` делал reconciler флапающим, а fail-stuck — не доходящим).
"""
...
def sweep_clarifications() -> int:
    async def _run() -> int:
        # worker_redis_scope: _sweep_clarifications→fail_job→transition→publish_event дёргает
        # Redis — per-task клиент в этом loop'е (ADR-019 §Fix), не глобальный ASGI-пул.
        async with worker_redis_scope(), task_engine_scope() as sessionmaker:
...
def reconcile_stuck() -> int:
    async def _run() -> int:
        # worker_redis_scope: fail-stuck/wall-clock ветви зовут fail_job→transition→publish_event
        # (Redis) — per-task клиент в этом loop'е (ADR-019 §Fix). Без него `RuntimeError: Event
        # loop is closed` делал reconciler флапающим, а терминализацию stuck-джобы — не доходящей.
        async with worker_redis_scope(), task_engine_scope() as sessionmaker:
...
def refresh_metrics() -> None:
    ...
    observability §7.0/§7.1 п.5: collector.refresh_queue_depth дёргает get_redis() (async-Redis,
    LLEN брокера). Без активного worker_redis_scope get_redis() вернул бы клиент глобального
    ASGI-пула, привязанного к чужому/закрытому loop'у → `RuntimeError: Future attached to a
    different loop` / `Event loop is closed` (ровно loop-affinity анти-паттерн, metrics.refresh
    назван в §7.1 п.5). Тело оборачивается в worker_redis_scope() — per-task Redis-клиент в этом
    же asyncio.run-loop'е, который подхватит get_redis() внутри refresh_queue_depth.
    """
